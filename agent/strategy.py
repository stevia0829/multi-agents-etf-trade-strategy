...
def filter_moment_rank(stock_pool, days, ll, hh, show_print=True):
    ...
    # 创建一个DataFrame来存储股票的年化收益率、R²值和综合得分
    scores_data = pd.DataFrame(index=stock_pool, columns=["annualized_returns", "r2", "score"])
    current_data = get_current_data()
    print_info = {}
    filtered_out = []  # 记录被过滤掉的股票
    high_score_stocks = []  # 记录超过阈值的股票

    for stock in stock_pool:
        # 获取过去 days 的收盘价历史数据
        prices = attribute_history(stock, days, '1d', ['close'])['close']
        ...
        try:
            ...
            scores_data.loc[stock, "annualized_returns"] = annualized_returns

            # 计算R2
            ss_res = np.sum(weights * (y - (slope * x + intercept)) ** 2)
            ss_tot = np.sum(weights * (y - np.mean(y)) ** 2)
            r2 = 1 - ss_res / ss_tot if ss_tot else 0
            scores_data.loc[stock, "r2"] = r2

            # 计算得分
            score = annualized_returns * r2
            scores_data.loc[stock, "score"] = score
            ...
            print_info[stock_name] = score

        except Exception as e:
            log.warning(f"计算股票 {stock} 得分时出错: {e}")
            continue
    ...
    # 分别应用上下限过滤
    above_min = scores_data.query(f"score >= {ll}")
    below_max = scores_data.query(f"score <= {hh}")
    
    # 检查是否有股票超过阈值
    above_threshold = scores_data.query(f"score > {hh}")
    
    if len(above_threshold) > 0:
        # 有股票超过阈值，执行新逻辑
        high_score_stocks = above_threshold.index.tolist()
        log.info(f"发现 {len(high_score_stocks)} 只股票超过阈值: {[get_security_info(stock).display_name for stock in high_score_stocks]}")
        
        # 计算前一天的分数
        yesterday_scores = {}
        for stock in high_score_stocks:
            # 获取过去 days+1 天的收盘价历史数据，以便计算t-1日分数
            prices = attribute_history(stock, days + 1, '1d', ['close'])['close']
            
            if prices.empty or len(prices) < days + 1:
                continue

            # 计算t-1日的分数（使用前days天的数据）
            yesterday_prices = prices[:-1]  # 去掉最后一天，保留前days天
            ...
            try:
                ...
                yesterday_scores[stock] = yesterday_score
                
            except Exception as e:
                log.warning(f"计算股票 {stock} t-1日得分时出错: {e}")
                continue
        ...
        qualified_stocks = []
        for stock in high_score_stocks:
            if stock in g.yesterday_scores:
                t_day_score = scores_data.loc[stock, "score"]
                t_minus_1_score = g.yesterday_scores[stock]
                
                # 检查t日分数是否大于等于t-1日分数的1.5倍
                if t_day_score >= t_minus_1_score * g.score_threshold_multiplier:
                    qualified_stocks.append(stock)
                    log.info(f"{get_security_info(stock).display_name}({stock}): t日分数 {t_day_score:.4f} >= t-1日分数 {t_minus_1_score:.4f} * {g.score_threshold_multiplier} = {t_minus_1_score * g.score_threshold_multiplier:.4f}")
                else:
                    log.info(f"{get_security_info(stock).display_name}({stock}): t日分数 {t_day_score:.4f} < t-1日分数 {t_minus_1_score:.4f} * {g.score_threshold_multiplier} = {t_minus_1_score * g.score_threshold_multiplier:.4f}，不进入排名")
            else:
                # 如果没有前一天的分数，则使用原逻辑
                qualified_stocks.append(stock)
        ...
        # 对符合条件的股票按分数降序排列
        if qualified_stocks:
            qualified_data = scores_data.loc[qualified_stocks]
            valid_data = qualified_data.sort_values(by="score", ascending=False)
        else:
            # 如果没有符合条件的股票，按照原逻辑处理
            valid_data = scores_data.query(
                f"{ll} <= score <= {hh}"  # 仅保留符合范围的股票
    ...
    if high_score_stocks:
        log.info(f"超过阈值({hh})的股票: {', '.join([f'{get_security_info(stock).display_name}({stock})' for stock in high_score_stocks])}")

    # 打印排名靠前的股票
    top_stocks_info = []
    for stock_code in valid_data.index.tolist():
        stock_name = get_security_info(stock_code).display_name
        score_val = print_info.get(stock_name, 'N/A')
        top_stocks_info.append(f"{stock_name} ({score_val:.4f})")
        
    log.info("股票动量评分排名: {}".format(' > '.join(top_stocks_info)))
...
def get_etf_rank(context, etf_pool):
    ...
    # 初始化局部变量，用于追踪当前批次中的最高分
    max_observed_score = float('-inf')
    ...
    if len(above_threshold) > 0:
        ...
        if qualified_etfs:
            ...
            valid_data = data.query(f"{g.min_score} <= score <= {g.max_score}") # 仅保留符合范围的ETF
            valid_data = valid_data.sort_values(by="score", ascending=False)
    else:
        # 没有ETF超过阈值，沿用原逻辑
        valid_data = data.query(f"{g.min_score} <= score <= {g.max_score}") # 仅保留符合范围的ETF
        valid_data = valid_data.sort_values(by="score", ascending=False)
    ...
    # 打印排名靠前的ETF
    top_etfs_info = []
    # 假设 print_info 是基于 print_data 构建的，这里进行修正
    # 注意：原代码中的 print_info 可能是股票部分的变量，这里应使用 print_data
    for etf_code in valid_data.index.tolist()[:5]: # 假设只取前5个
        etf_name = get_security_info(etf_code).display_name
        # 从 print_data 中获取分数，如果不存在则为 'N/A'
